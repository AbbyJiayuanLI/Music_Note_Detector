...
%     if (abs(noteTrack(i) - noteTrack(i-1)) > 15)
%         noteTrack(i) = noteTrack(i-1);
%     end
% end
...
%% Play Sound
% 
% freqTrack = zeros(numRound, 1);
% for i=1:1:length(noteTrack)
%     note = (12*log2(freq/440));
%     freqTrack(i) = 440*2^(noteTrack(i)/12);
% end
% 
% music_reconstruct = resample(freqTrack, round(fs/(length(noteTrack)/(N/fs))), 1);
